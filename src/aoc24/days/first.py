...
def part_b(data_path: pathlib.Path) -> int:
    df = pd.read_table(data_path, sep=r"\s+", header=None)
    series = list(df[1])
    df[2] = df.apply(lambda _: series, axis=1)  # this doesn't work as the indexing be smart // map
    return df.apply(lambda x: x[0] * sum(1 for item in x[2] if item == x[0]), axis=1).sum()
